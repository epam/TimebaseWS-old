...
import com.epam.deltix.tbwg.webapp.utils.qql.FilteredMessageSource;
...
public class SelectBuilder2 implements Reusable {
    ...
    private List<IdentityKey> ids = null;
    ...
    public SelectBuilder2 setIdentitiesList(List<IdentityKey> ids) {
        this.ids = ids.isEmpty() ? null : ids;
        return this;
    }

    public SelectBuilder2 setIdentities(IdentityKey... ids) {
        return setIdentitiesList(Arrays.asList(ids));
    }

    public SelectBuilder2 setSymbols(List<String> symbols) {
        return setIdentities(TBWGUtils.match(stream, symbols));
    }
        ...
        if (db != null) {
            if (runtimeFilters.isEmpty()) {
                ...
                return db.executeQuery(
                        ...
                        null,
                        ids != null ? TBWGUtils.collectCharSequence(ids.toArray(new IdentityKey[0])) : null,
                        startTime
                );
            } else {
                String query = toString();
