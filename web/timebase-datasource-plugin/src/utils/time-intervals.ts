...
export const getCurrentInterval = (intervals: any, selectedInterval: number, isChangeCurrencyInterval: boolean) => {
  return intervals != null
    ? selectedInterval === Number.MIN_VALUE
      ? 'NONE'
      : isNaN(selectedInterval) || selectedInterval == null || isChangeCurrencyInterval
      ? 'MIN INTERVAL'
      : intervals.find((interval: any) => interval.value === selectedInterval)?.label || 'MIN INTERVAL'
    : null;
};
