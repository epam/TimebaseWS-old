...
export class FiltersPanelComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.streamDetailsEffects
      ...
      .subscribe(action => {
          if (action.payload.symbols && action.payload.symbols.length) {
            this.symbols = [...action.payload.symbols];
          }
        },
      );
  ...
  onValueChange(event: any) {
    // this.currentDateTimeFullHoursFrom = event;
    //  console.log(this.currentDateTimeFullHoursFrom);
  }
  ...
  openFilterModalWithComponent() {
    ...
    this.bsModalRef.content.onClear = () => {
      ...
      //   this.streamDetailsStore.dispatch(new StreamDetailsActions.ClearGFilterState());
    };
