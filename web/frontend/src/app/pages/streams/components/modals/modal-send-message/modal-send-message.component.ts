...
import { SchemaAllTypeModel }                                            from '../../../../../shared/models/schema.type.model';
import {
  dateToUTC,
  hdDateTZ,
...
export class ModalSendMessageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private commonMessageFormdDstroy$: Subject<boolean> = new Subject();
  public showEditor = false;
  public simpleConfig: SimpleColumnModel[];
  public commonSimpleConfig: SimpleColumnModel[];
  private allConfig: SchemaAllTypeModel[];
  public formData?: any;
  public editorData;
  ...
  ngOnInit(): void {
    if (this.stream) {
      this.schemaDataService
        ...
        .pipe(
          switchMap(([allConfig, simpleConfig]: [SchemaAllTypeModel[], SimpleColumnModel[]]) => {
        ...
        .subscribe(([simpleConfig, symbols]: [SimpleColumnModel[], string[]]) => {
          ...
          this.commonMessageForm = this.generateForm(this.commonSimpleConfig, this.commonMessageFormdDstroy$);

          if (this.commonMessageForm.get('$type')) this.commonMessageForm.get('$type').setValue((this.commonSimpleConfig.find(config => config.field === '$type')).controlCollection[0].key);
          if (this.commonMessageForm.get('symbol')) this.commonMessageForm.get('symbol').setValue((this.commonSimpleConfig.find(config => config.field === 'symbol')).controlCollection[0].key);
          ...
          if (this.formData) {
            ...
            if (this.commonMessageForm.get('$type')) {
              const key = (this.commonSimpleConfig.find(config => config.field === '$type')).controlCollection[0].key,
  ...
  private get$TypeCollection(simpleConfig: SimpleColumnModel[]): {
    ...
    return Object.keys(simpleConfig).filter(configKey => (simpleConfig[configKey].children && simpleConfig[configKey].children.length)).map(configKey => ({
      key: simpleConfig[configKey].field,
      title: simpleConfig[configKey].headerName,
    }));
  }
