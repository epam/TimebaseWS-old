...
import { hdDateTZ } from './utils/timezone.utils';
...
export const timeFormatsSupported = [
    'HH:mm:ss',
    'HH:mm:ss tt',
    'HH:mm:ss.fff',
    'HH:mm:ss.ffffff',
...
export function getLocaleDateString() {
    ...
    let format = formats[navigator.language] || formats['en-US'];
...
export const DEFAULT_DATE_FORMAT = getLocaleDateString();
export const DEFAULT_TIME_FORMAT = timeFormatsSupported[2];
...
export function formatHDate(data: string, filter_date_format?: string[], filter_time_format?: string[], filter_timezone?: any[]) {
    ...
    let time_format = 'HH:mm:ss fff';
    if (filter_date_format && filter_date_format.length) {
        date_format = filter_date_format[0];
    }
